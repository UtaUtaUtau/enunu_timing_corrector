...
def correct_timing(score_list, timing_list, vowels):
    ...
    for i in range(1, len(score_list)): # Overlapping label correct
        # TODO: Remove the possibility of end being equal or earlier than start
        curr = score_list[i]
        prev = score_list[i-1]
        if curr[0].start != prev[-1].end: # If overlapping
            # Correct end of previous note
            prev[-1].end = curr[0].start
            if len(prev) == 1 or prev[-1].phone in vowels:
                # Skip if previous note only has one phoneme or the last phoneme is a vowel
                continue
            # Correct coda positioning
            ...
            for j in range(len(prev)-2, -1, -1):
                ...
                if prev[j].phone in vowels:
                    # Stop if a vowel is seen. Putting vowels in one note is now... kind of bad to do from this.
                    break
